...
export class AuthService {
  ...
  constructor() {
    ...
    this.#user$.subscribe(async (aUser: User | null) => {
      ...
      if (aUser) {
        try {
          let idTokenResult = await this.auth.currentUser?.getIdTokenResult();
          this.logger.debug('claims:', idTokenResult);
          let role = UserRole.User;
          let team = '';
          let hasClaims = typeof idTokenResult?.claims[CustomClaimField.role] === 'number';
          if (hasClaims) {
            ...
            this.logger.debug('custom claims not present');
  ...
  private async saveMessagingDeviceToken(): Promise<boolean> {
    this.logger.debug('AuthService::saveMessagingDeviceToken');
    try {
